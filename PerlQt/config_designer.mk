...
_HEADERS  := $(filter-out %/extension_global.h,                  \
                   $(wildcard                                    \
                       $(addprefix $(_HEADER_DIR)/,              \
                           $(addsuffix /*.h, $(_QT)))))
# common enums declared in qnamespace.h
_HEADERS  += $(_HEADER_DIR)/QtCore/qnamespace.h
...
# this line will be scaned by script/gen_makefile_pl.pl
# no make variables inside, keep value in one line
DESIGNER_DEFINES := -DQT_SHARED -DQT_NO_DEBUG
...
EXTRA_DEFINES := -DQT_NO_KEYWORDS -DQ_DECL_EXPORT="Q_DECL_EXPORT" -DQDESIGNER_EXTENSION_EXPORT="Q_DECL_EXPORT"

ALL_DEFINES   := $(DESIGNER_DEFINES) $(EXTRA_DEFINES)
...
# no make variables inside, keep value in one line
LDFLAGS := -L/usr/lib/qt4 -lQtDesigner -lQtCore -lQtXml
