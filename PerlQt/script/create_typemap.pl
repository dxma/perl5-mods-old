...
sub main {
    ...
    foreach my $n (keys %META_DICTIONARY) {
        if (exists $META_DICTIONARY{$n}->{ISA}) {
            foreach my $p (@{$META_DICTIONARY{$n}->{ISA}}) {
                push @{$type{$n}}, $p->{NAME} if $p->{RELATIONSHIP} eq 'public';
            }
        }
    }
    # collect function parameter/return type
